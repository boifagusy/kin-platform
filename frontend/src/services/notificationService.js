...
export const startNotificationChecker = async (phone) => {
  try {
    ...
    checkerInterval = setInterval(async () => {
      try {
        const res = await fetch(`${API_BASE}/trusted-contact/notifications/${encodeURIComponent(phone)}`);
        ...
        if (data.success && Array.isArray(data.data) && data.data.length > 0) {
          ...
          for (const notif of data.data) {
            await LocalNotifications.schedule({
              notifications: [
                {
                  title: "🚨 KIN Safety Alert",
                  body: notif.message || "A trusted contact may need assistance.",

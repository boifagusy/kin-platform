...
https://kin.app`;
  ...
  const fetchContact = async () => {
    try {
      ...
      if (data.success) {
        const contactData = data.data ?? null;
        setContact(contactData);
        setUsed(contactData ? 1 : 0);
        setCanAdd(!contactData);
        setCanRemove(true);
        setDaysUntilRemoval(0);
      }
    } catch (error) {
  ...
  const addContact = async () => {
    ...
    try {
      ...
      if (result.state === "SENT") {
        ...
        setInviteSent(false);
        if (navigator.share) { navigator.share({ title: "KIN Trusted Contact Invite", text: inviteMessage }).catch(err => console.error("Share failed:", err)); } else { setShowInviteModal(true); }

...
function NetworkScreenV2() {
  ...
  // ✅ NEW: Track if contact is verified
    ...
    try {
      ...
      if (data.success) {
        ...
        if (contactData) {
          ...
          setIsVerified(contactData.verified === true);
          // If contact exists and is not verified, show share button
          if (contactData.verified !== true) {
            setContactAdded(true);
            setNewContactName(contactData.name);
            setNewContactPhone(contactData.phone);
          }
        }
      }
    } catch (error) {
  ...
  // ✅ NEW: Share Invitation using Web Share API
  const handleShareInvite = async () => {
    const token = verificationToken || contact?.verification_token || Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
    const verifyLink = `${API_BASE}/trusted-contact/verify/${token}`;
    const userName = localStorage.getItem("kin_user_name") || "A user";
    const contactName = newContactName || contact?.name || "there";

    const message = `Hi ${contactName},

${userName} has added you as a trusted contact on KIN, a personal safety app. This means you may receive an alert if they miss a scheduled check-in or get alerted in case of emergency.
...
📲 Get KIN: https://kin.app

Thanks for helping keep ${userName} safe.
— The KIN Team`;
    ...
    if (navigator.share) {
      ...
      try {
        ...
        setShareMessage("✅ Invitation copied to clipboard! Share it with your contact.");
        setTimeout(() => setShareMessage(""), 3000);
      } catch (err) {
        // Final fallback
        ...
        setShareMessage("✅ Invitation copied to clipboard!");
        setTimeout(() => setShareMessage(""), 3000);
      }
    }
  };
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#E9ECEF]">
            ...
            <div className={`p-3 rounded-xl mb-4 text-sm ${
              ...
              {isVerified
                ...
                : "📤 Share the invitation with this contact to complete the setup."}
            </div>
            ...
            {/* ✅ SINGLE SHARE BUTTON — Stays visible until verified */}
            {!isVerified && (
              <button
                onClick={handleShareInvite}
                className="w-full h-12 rounded-xl bg-[#D4A017] text-white font-semibold text-base shadow-lg shadow-[#D4A017]/20 hover:opacity-90 active:scale-95 transition-all flex items-center justify-center gap-3 mb-3"
              >
                <FaShareAlt />
                Share Invitation
              </button>
            ...
            {isVerified && (
              <div className="w-full h-12 rounded-xl bg-green-100 text-green-700 font-semibold text-base flex items-center justify-center gap-3 mb-3">
                <FaCheckCircle />
                Verified ✓
              </div>
            )}

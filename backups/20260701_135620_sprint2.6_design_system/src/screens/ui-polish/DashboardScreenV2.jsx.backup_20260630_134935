...
function DashboardScreenV2() {
  ...
  const handleTaskClick = (task) => {
    if (task.id === "location") {
      getCurrentLocation()
        .then(() => setLocationPermissionGranted(true))
        .catch(() => setLocationPermissionGranted(false));
    } else if (task.id === "safe_zones") {
  ...
  const rawTasks = dashboard?.pending_tasks || [];
  const tasks = rawTasks.filter(t => t.id !== "location" || !locationPermissionGranted);
      ...
      {showAddZoneModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }}>
          <div style={{ background: "white", borderRadius: 20, padding: 24, width: "100%", maxWidth: 380 }}>
            <h3 style={{ fontWeight: "bold", fontSize: 18, color: "#1A5632", marginBottom: 16 }}>Add Safe Zone</h3>
            ...
            <input
              ...
              style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #e5e7eb", marginBottom: 12, fontSize: 14, boxSizing: "border-box" }}
            />

            <label style={{ fontSize: 12, color: "#6b7280", display: "block", marginBottom: 4 }}>Address (optional)</label>
            <input
              type="text"
              value={zoneAddress}
              onChange={(e) => setZoneAddress(e.target.value)}
              placeholder="Street address"
              style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid #e5e7eb", marginBottom: 16, fontSize: 14, boxSizing: "border-box" }}
            />

            {zoneError && (
              <p style={{ color: "#DC3545", fontSize: 13, marginBottom: 12 }}>{zoneError}</p>
            )}

            <div style={{ display: "flex", gap: 10 }}>
              <button
                onClick={() => { setShowAddZoneModal(false); setZoneError(""); }}
                style={{ flex: 1, padding: "12px", borderRadius: 12, border: "none", background: "#F3F4F6", color: "#374151", fontWeight: "600", cursor: "pointer" }}
              >
                Cancel
              </button>
              <button
                onClick={handleAddZone}
                disabled={!zoneName.trim() || zoneSaving}
                style={{ flex: 1, padding: "12px", borderRadius: 12, border: "none", background: !zoneName.trim() || zoneSaving ? "#B7D4BF" : "#1A5632", color: "white", fontWeight: "600", cursor: !zoneName.trim() || zoneSaving ? "not-allowed" : "pointer" }}
              >

...
class RecoveryController extends Controller
{
    ...
    public function dashboardData(): View
    {
        $stats = $this->engine->getStats();
        ...
        $active = \App\Models\RecoveryAttempt::where('status', 'running')->count();
        $avgTime = \App\Models\RecoveryAttempt::where('status', 'success')
            ->whereNotNull('duration_ms')
            ->avg('duration_ms') ?? 0;
        $escalated = \App\Models\RecoveryAttempt::where('escalated', true)->count();

        return view('recovery.dashboard', [
            'stats' => $stats,
            'recent' => $recent,
            'active' => $active,
            'avgTime' => round($avgTime / 1000, 2),
            'escalated' => $escalated,

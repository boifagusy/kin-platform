...
class AuthController extends Controller
{
    public function login(Request $request)
    {
        ...
        if ($existing) {
            ...
            if ($secondsSinceLastSend < $cooldownSeconds) {
                ...
                // Still return the same success shape so this can't be used
                // to enumerate accounts or probe cooldown state.
    ...
    /**
     * LOGIN PIN - Modified to detect duress PIN
     *
     * If user enters duress PIN:
     * - Login appears normal (returns same response)
     * - Silent SOS is triggered in background
     * - Trusted contacts are notified
     * - Activity log records duress event
     */
    public function loginPin(Request $request, LoginPinAction $action)
    {
        ...
        if ($isDuressPin) {
            ...
            // Return normal login response (user thinks everything is normal)
            return response()->json([
                'success' => true,
                'user_id' => $user->id,
                'onboarding_completed' => $user->onboarding_completed,
                'is_duress' => false, // Don't reveal to frontend
            ], 200);
        }
    ...
    /**
     * Trigger silent SOS when duress PIN is used
     */
    private function triggerDuressSos(User $user)
    {
        ...
        // Log to Laravel log for now (SMS integration will come later)
    ...
    /**
     * Save user details (phone, full_name, email)
     */
    ...
    /**
     * Handle duress PIN verification with hash
     */
    private function verifyDuressPin($user, $pin)
    {
        ...
        if (!Hash::check($pin, $user->duress_pin_hash)) {
            // Log failed attempt
            Log::warning('Invalid duress PIN attempt', [
                'user_id' => $user->id,
                'ip' => request()->ip(),
            ]);
            return false;
    ...
    /**
     * Updated loginPin with duress detection
     */
    public function loginPin(Request $request, LoginPinAction $action)
    {
        $request->validate([
        ...
        if ($this->verifyDuressPin($user, $request->pin)) {
            // Valid duress PIN - trigger silent SOS
            $this->triggerDuressSos($user);
            
            // Log but don't expose
            Log::info('Duress PIN used', [
                'user_id' => $user->id,
                'silent' => true,
            ]);
            ...
            return ApiResponse::success([
                'user' => $user,
                'token' => $user->createToken('kin-app')->plainTextToken,
            ], 'Login successful');
        }

        // Regular PIN verification
        if (!Hash::check($request->pin, $user->login_pin_hash)) {
            return ApiResponse::error('Invalid PIN', 401);
        }

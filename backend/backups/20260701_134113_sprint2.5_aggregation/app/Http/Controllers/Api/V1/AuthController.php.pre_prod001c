...
class AuthController extends Controller
{
    ...
    private function triggerDuressSos(User $user)
    {
        ...
        $sos = SosEvent::create([
            'user_id' => $user->id,
            'latitude' => null, // Will be updated when location available
            'longitude' => null,
            'triggered_at' => now(),
            'is_duress' => true, // Mark as duress
        ]);
    ...
    public function userDetails(Request $request, SaveUserDetailsAction $action)
    {
        $result = $action->execute($request->input('phone'), $request->input('full_name'), $request->input('email'));
        if (!$result['success']) {
            return response()->json(['error' => $result['error']], 422);
        }
        return response()->json($result);

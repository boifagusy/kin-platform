...
Route::prefix('v1')->group(function () {
    ...
    // ⚠️ PUBLIC FOR MVP TESTING ONLY — wrap in auth:sanctum before production (see Step 5)
    Route::get('/incidents', [IncidentController::class, 'index']);
    Route::get('/incidents/{id}', [IncidentController::class, 'show']);
    Route::post('/incidents/{id}/resolve', [IncidentController::class, 'markResolved']);

    // --- ROUTE A: use this line ONLY if controller signature is notifications($contactPhone) ---
    Route::get('/trusted-contact/notifications/{phone}', [IncidentController::class, 'notifications']);

    // --- ROUTE B: use this line ONLY if controller signature is notifications(Request $request) ---
    // Route::get('/trusted-contact/notifications', [IncidentController::class, 'notifications']);
